...
public final class PatternMatcher {
    ...
    public static Optional<Match> match(Pattern pattern, int patternIndex, List<Token> tokens, SlotResolver resolver, ParseScope scope) {
        ...
        boolean matched = matcher.match(pattern.root(), 0, position -> position == tokens.size());
        if (!matched) {
    ...
    private boolean match(PatternElement element, int position, Continuation next) {
        return switch (element) {
            case PatternElement.Literal literal -> position < tokens.size() && tokens.get(position).is(literal.word()) && next.run(position + 1);
            case PatternElement.Sequence sequence -> matchSequence(sequence.elements(), 0, position, next);
            case PatternElement.Optional optional -> match(optional.element(), position, next) || next.run(position);
            case PatternElement.Choice choice -> matchChoice(choice, position, next);
            case PatternElement.Slot slot -> matchSlot(slot, position, next);
        };
    }

    private boolean matchSequence(List<PatternElement> elements, int index, int position, Continuation next) {
        if (index == elements.size()) {
            return next.run(position);
        }
        return match(elements.get(index), position, p -> matchSequence(elements, index + 1, p, next));
    }

    private boolean matchChoice(PatternElement.Choice choice, int position, Continuation next) {
        for (PatternElement.Branch branch : choice.branches()) {
            boolean tagged = !branch.tag().isEmpty();
            if (tagged) {
                tags.push(branch.tag());
            }
            if (match(branch.element(), position, next)) {
                return true;
            }
            if (tagged) {
                tags.pop();
    ...
    private boolean matchSlot(PatternElement.Slot slot, int position, Continuation next) {
        for (int end = tokens.size(); end > position; end--) {
